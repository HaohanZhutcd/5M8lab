        ...
        if ( block_mae > mae_t )
            ...
            for jj = search_range_y
                for ii = search_range_x
                    ...
                    % In this case, 
                    % MAE = absolute(X1 - X2) / total sample sizes
                    error_ = mean(abs(ref_block(:) - other_block(:))); 

                    % enumerate the error, obtain the minimum error
                    % the min error list was stored in 'min_error_'
                    % if the I(i,j) get a smaller error with DFD,
